...
//    @BindView(R.id.friend_count) TextView friendCount;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        StatusBarUtil.setTransparentForImageView(ProfileActivity.this, dp);
...
//            FOR NOTIFICATIONS
        ...
        socialUtils = SocialUtils.get(currentUser.getUid(), friendUserId, this);
    ...
    private void updateUI() {
        try {
            DatabaseReference usersDatabase = FirebaseDatabase.getInstance().getReference().child(USERS).child(friendUserId);
            usersDatabase.addValueEventListener(new CustomValueEventListener() {
                @Override public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot != null && dp != null && name != null && status != null) {
    ...
    private void updateActionButton() {
        try {
            root.child(FRIEND_REQUESTS).child(currentUser.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    progressBar.setVisibility(View.GONE);
                    if (dataSnapshot.hasChild(friendUserId)) {
                        String requestType = requireNonNull(dataSnapshot.child(friendUserId).child(REQUEST_TYPE).getValue()).toString();
...
//                        Check if we are already friends or not
                        root.child(FRIENDS).child(currentUser.getUid()).addListenerForSingleValueEvent(new CustomValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot dataSnapshot) {
//                                check if friend if there in out friends list
    ...
    @Override public void updateState(@SocialState int state) {
        if (actionButton != null && deleteRequestButton != null) {
